# Projects List

Ниже приведён список всех моих pet-проектов и практических работ.  
Обучающие проекты находятся внутри этого репозитория.  
Практические и реальные проекты размещены в отдельных репозиториях.

---

## 1. Stepik - тестирование форм авторизации
**Тип:** учебный проект (дипломная работа)  
**Артефакты:** чек-листы, тест-кейсы, баг-репорты  
**Статус:** проект хранится в этом репозитории  
**Папка:** [/stepik-auth-testing](https://github.com/AnastasiyaPolyanskaya/qa-study-projects/tree/main/stepik-auth-testing)

---

## 2. Бесплатное-обучение.рф
**Тип:** учебный проект  
**Артефакты:** тест-кейсы, чек-листы, исследовательское тестирование  
**Статус:** проект хранится в этом репозитории  
**Папка:** [/besplatnoe-obuchenie-testing](https://github.com/AnastasiyaPolyanskaya/qa-study-projects/tree/main/besplatnoe-obuchenie-testing)

---

## 3. Техники тест-дизайна
**Тип:** самостоятельная практика  
**Артефакты:** классы эквивалентности, граничные значения, таблицы решений  
**Статус:** проект хранится в этом репозитории  
**Папка:** [/test-design-examples](https://github.com/AnastasiyaPolyanskaya/qa-study-projects/tree/main/test-design-examples)
---

## 4. IVI Beta Testing
**Тип:** реальное бета-тестирование  
**Артефакты:** баг-репорты, чек-листы, сценарии тестирования  
**Репозиторий:** [IVI-Beta-Testing](https://github.com/AnastasiyaPolyanskaya/IVI-Beta-Testing)

---

## 5. VK Testers (VK Beta)
**Тип:** реальное бета-тестирование  
**Артефакты:** баг-репорты, UX-замечания  
**Репозиторий:** [VK-Beta-Testing](https://github.com/AnastasiyaPolyanskaya/VK-Beta-Testing)

---

## 6. Python practice
**Тип:** учебная практика  
**Описание:** задачи по циклам, строкам, функциям, условиям  
**Репозиторий:** [Python-practice](https://github.com/AnastasiyaPolyanskaya/qa-study-projects/tree/main/python-practice)

---

## 7. SQL practice
**Тип:** учебная практика  
**Описание:** SELECT, JOIN, группировки, подзапросы  
**Репозиторий:** [sql-practice](https://github.com/AnastasiyaPolyanskaya/qa-study-projects/tree/main/sql-practice)


## 8. Wink Beta — тестирование главной страницы (UI)
**Тип:** учебный проект  
**Артефакты:** тест-кейсы, баг-репорты, скриншоты  
**Описание:** исследовательское UI-тестирование мобильной версии сайта Wink Beta. Проверялась корректность отображения элементов главной страницы и адаптивность блоков.  
**Статус:** проект хранится в этом репозитории  
**Папка:** [/wink-beta-ui-testing](https://github.com/AnastasiyaPolyanskaya/qa-study-projects/tree/main/wink-beta-ui)

## 9. User Journey Testing — holodilnik.ru
**Тип:** практический проект  
**Артефакты:** пользовательские пути, комментарии, UX-замечания, ссылки на видео  
**Описание:** исследовательское, функциональное и UI/UX-тестирование пользовательских путей интернет-магазина holodilnik.ru. Проверялись сценарии от каталога и карточки товара до этапа оплаты, включая авторизацию, доставку и оплату банковской картой.  
**Статус:** проект хранится в этом репозитории  
**Папка:** [/User-Journey-Testing-holodilnik.ru](https://github.com/AnastasiyaPolyanskaya/User-Journey-Testing-holodilnik.ru)

